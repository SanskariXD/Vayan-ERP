...
class DuplicateFilterPipeline:
    ...
    def __init__(self):
        self.seen_ids = {
            ...
            'HandloomPinItem': set(),      # Handloom demand-sensing items
        }
    ...
    def get_unique_identifier(self, adapter, item_type):
        ...
        elif item_type == 'HandloomPinItem':
            # Deduplicate on pin_id + search_query so the same pin can appear
            # for different handloom queries without being suppressed.
            return f"{adapter.get('pin_id', '')}_{adapter.get('search_query_used', '')}"
        else:
...
class HandloomJsonPipeline:
    """
    Writes raw HandloomPinItems to output/raw_handloom_pins.json.

    This file is the direct input for rank_trends.py:

        Spider  →  raw_handloom_pins.json  →  rank_trends.py  →  ranked_trends.json
                                                                       ↓
                                                            Next.js API Route

    The handloom_search spider also has a FEEDS setting that writes the same
    file via the built-in feed exporter. This pipeline acts as a secondary,
    always-reliable fallback that doesn't depend on FEEDS being enabled.

    Registration in settings.py:
        ITEM_PIPELINES = {
            'pinterest_scraper.pipelines.HandloomJsonPipeline': 400,
            ...other pipelines...
        }
    """

    OUTPUT_PATH = "output/raw_handloom_pins.json"

    def __init__(self):
        self._items = []

    def open_spider(self, spider):
        """Only activate for the handloom_search spider."""
        if spider.name != "handloom_search":
            return

        import os
        os.makedirs("output", exist_ok=True)
        spider.logger.info(
            f"📁 HandloomJsonPipeline: will write to '{self.OUTPUT_PATH}'"
        )

    def process_item(self, item, spider):
        """Collect HandloomPinItems; skip all other item types silently."""
        if item.__class__.__name__ != "HandloomPinItem":
            return item  # Pass-through non-handloom items unchanged

        from itemadapter import ItemAdapter
        self._items.append(dict(ItemAdapter(item)))
        return item

    def close_spider(self, spider):
        """Write accumulated items to JSON at spider close."""
        if spider.name != "handloom_search":
            return
        ...
        spider.logger.info(
            f"✅ HandloomJsonPipeline: wrote {len(self._items)} pins "
            f"to '{self.OUTPUT_PATH}'"
        )
        spider.logger.info(
            "▶  Next step: python rank_trends.py"
        )