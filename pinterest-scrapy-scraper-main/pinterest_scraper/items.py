...
class HandloomPinItem(scrapy.Item):
    """
    Demand-sensing item for the handloom cooperative pipeline.
    Extracted by HandloomSearchSpider, consumed by rank_trends.py.

    Field contract (must match rank_trends.py column expectations):
      - pin_id           : str   — unique Pinterest pin identifier
      - image_url        : str   — highest-resolution pinimg URL available
      - title            : str   — pin title or alt-text
      - description      : str   — pin description (may be empty)
      - saves            : int   — primary engagement signal (most critical)
      - repins           : int   — secondary signal; Pinterest re-save count
      - comments         : int   — tertiary signal
      - engagement_score : int   — computed: (saves * 2) + repins
      - search_query_used: str   — which handloom keyword triggered this pin
      - region_tag       : str   — e.g. 'South India', 'Varanasi', 'Global'
      - pin_url          : str   — full pinterest.com/pin/... URL
    ...
    # ---- Media ----
    image_url         = scrapy.Field()   # High-res preferred (736x or originals/)

    # ---- Engagement metrics (THE critical fields) ----
    saves             = scrapy.Field()   # Pinterest saves (most important)
    repins            = scrapy.Field()   # Re-pins / re-saves
    comments          = scrapy.Field()   # Comment count

    # ---- Computed by spider before yield ----
    engagement_score  = scrapy.Field()   # (saves * 2) + repins

    # ---- Handloom-specific context ----
    search_query_used = scrapy.Field()   # The exact HANDLOOM_QUERIES entry used
    region_tag        = scrapy.Field()   # Inferred from keyword (South India, etc.)

    # ---- Frontend handoff flags (set to defaults by rank_trends.py) ----
    # These fields are NOT populated by the spider; rank_trends.py injects them.
    # Declared here for documentation only.
